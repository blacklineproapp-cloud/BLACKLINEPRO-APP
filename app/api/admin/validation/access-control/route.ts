...
export async function GET(req: Request) {
  try {
    ...
    console.log('[AccessControl] Iniciando validação de controle de acesso...');

    // =========================================================================
    // 1. BUSCAR TODOS OS USUÁRIOS DO CLERK
    // =========================================================================
    const allClerkUsers = await ClerkService.getAllUsersActivity();
    console.log(`[AccessControl] Encontrados ${allClerkUsers.length} usuários no Clerk`);
    ...
    console.log(`[AccessControl] Encontrados ${supabaseUsers?.length || 0} usuários no Supabase`);
    ...
    for (const clerkUser of allClerkUsers) {
      ...
      // ⚠️ PROBLEMA 1: Usuário ATIVO no app SEM pagamento e SEM grace/courtesy
      if (clerkUser.isActive && !shouldHaveAccess && !supabaseUser.is_blocked) {
        usersUsingAppWithoutPayment.push({
          ...
          severity: 'CRITICAL', // Acesso não autorizado
        });
      }

      // ⚠️ PROBLEMA 2: Usuário bloqueado mas DEVERIA ter acesso
      if (supabaseUser.is_blocked && shouldHaveAccess) {
        usersBlockedIncorrectly.push({
          ...
          plan: supabaseUser.plan,
          severity: 'HIGH', // Usuário pagante bloqueado
        });
      }

      // ⚠️ PROBLEMA 3: Usuário migrado SEM pagamento Asaas
      if (supabaseUser.migration_status === 'migrated' && !supabaseUser.asaas_subscription_id && !hasGracePeriod && !hasCourtesy) {
        migratedWithoutAsaasPayment.push({
          ...
          isBlocked: supabaseUser.is_blocked,
          plan: supabaseUser.plan,
          severity: 'HIGH', // Migração incompleta
        });
      }
    }

    // =========================================================================
    // 4. BUSCAR USUÁRIOS NO SUPABASE MAS NÃO NO CLERK
    // =========================================================================
    ...
    console.log(`[AccessControl] Usuários no Supabase mas não no Clerk: ${usersInSupabaseNotInClerk.length}`);
    ...
    console.error('[AccessControl] Erro ao validar controle de acesso:', error);
    return NextResponse.json(
      { error: 'Erro ao validar controle de acesso: ' + error.message },
      { status: 500 }
    );
  }
}
...
function calculateHealthScore(
  ...
  const maxPenalty = totalUsers * criticalWeight; // Pior caso: todos os usuários com problema crítico
