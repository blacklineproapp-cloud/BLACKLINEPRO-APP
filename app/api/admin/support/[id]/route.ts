...
export const GET = withAdminAuth(async (req, { userId }, routeCtx: RouteParams) => {
  ...
  if (ticketError) {
    logger.error('[Admin Support] Erro ao buscar ticket', { error: ticketError });
  ...
  if (userError || !ticketUser) {
    logger.error('[Admin Support] Erro ao buscar usuário do ticket', { error: userError });
  ...
  if (messagesError) {
    logger.error('[Admin Support] Erro ao buscar mensagens', { error: messagesError });
  }
 ...
 *
 * Actions disponíveis:
 * - change_plan: { plan: 'free' | 'ink' | 'pro' | 'studio' }
 * - unblock: {}
 * - clear_cache: {}
 * - reset_usage: {}
 */
...
async function executeAction(
  ...
  try {
    switch (action) {
      case 'change_plan': {
        const { plan } = data;
        if (!['free', 'ink', 'pro', 'studio'].includes(plan)) {
