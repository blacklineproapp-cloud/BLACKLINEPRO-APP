...
PROCESS THE IMAGE NOW:`;
  ...
  logger.info('[Refine Drawing] Iniciando refinamento com Gemini');
  ...
  const refinedImage = await retryGeminiAPI(async () => {
    ...
    if (parts) {
      for (const part of parts) {
        ...
        if (part.inlineData) {
          ...
          logger.info('[Refine Drawing] Imagem refinada recebida do Gemini');
    ...
    logger.error('[Refine Drawing] Gemini não retornou imagem', new Error(text.substring(0, 200)));
