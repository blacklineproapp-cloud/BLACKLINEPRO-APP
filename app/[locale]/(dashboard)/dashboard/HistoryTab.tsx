...
export default function IAGenGallery({ images, userId }: IAGenGalleryProps) {
  ...
  const handleDelete = async (imageId: string, e?: React.MouseEvent) => {
    e?.stopPropagation();
    if (!confirm(t('actions.deleteConfirmIA'))) return;
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
        <h2 className="text-zinc-300 font-semibold flex items-center gap-2 text-sm">
          <Clock size={16} /> {t('gallery.iaTitle')}
          {filteredImages.length !== images.length && (
            <span className="text-zinc-500 font-normal">
              ({filteredImages.length} {tCommon('of')} {images.length})
            </span>
          )}
        </h2>
                ...
                <div className="bg-zinc-900 p-2 lg:p-3 border-t border-zinc-800">
                  {editingImageId === image.id ? (
                    ...
                    <div className="flex items-center gap-1 group/name">
                      <h3 className="text-zinc-200 font-medium text-xs lg:text-sm truncate flex-1">{image.metadata.prompt || t('gallery.noDescription')}</h3>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
            ...
            <div className="flex items-center justify-center gap-4 mt-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-zinc-500">
                  {currentPage * IMAGES_PER_PAGE + 1}-{Math.min((currentPage + 1) * IMAGES_PER_PAGE, filteredImages.length)} {tCommon('of')} {filteredImages.length}
                </span>
              </div>
          ...
          >
            ...
            <div className="flex items-center justify-between p-4 border-b border-zinc-800">
              <div className="flex-1 min-w-0">
                <h3 className="text-white font-semibold truncate">{selectedImage.metadata.prompt || t('gallery.iaTitle')}</h3>
                <p className="text-zinc-500 text-xs mt-0.5">
                  {new Date(selectedImage.created_at).toLocaleDateString(locale)} 
