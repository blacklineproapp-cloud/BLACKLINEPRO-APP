  ...
  const handleDelete = async (projectId: string, e?: React.MouseEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(errorData.error || t('actions.deleteError'));
      }
    } catch (error: any) {
    ...
    <div className="p-4 lg:p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-6 lg:mb-8 bg-gradient-to-br from-zinc-900 to-zinc-950 border border-zinc-800 rounded-xl p-4 lg:p-6">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="flex items-center gap-3 lg:gap-4">
            ...
            <div>
              <h3 className="text-white font-semibold text-sm lg:text-base">{t('usage.current')}</h3>
          ...
          <div className="text-right">
            <div className="text-xs text-zinc-500">{tCommon('plan')}</div>
      ...
      {!isSubscribed && (
        <div className="mb-6 lg:mb-8 bg-gradient-to-r from-emerald-900/30 to-purple-900/30 border border-emerald-500/30 rounded-xl p-4 lg:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3 lg:gap-4">
            ...
            <div>
              <h3 className="text-white font-semibold text-sm lg:text-base">{t('upgrade.title')}</h3>
      ...
      <div className="mb-6 lg:mb-10">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl lg:text-3xl font-bold text-white mb-1">{t('title')}</h1>
      ...
      <div className="mb-8 lg:mb-12">
        <h2 className="text-zinc-300 font-semibold mb-4 lg:mb-6 text-xs lg:text-sm uppercase tracking-wider">{t('workflow.title')}</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
          <div className="bg-zinc-900 border border-zinc-800 p-4 lg:p-6 rounded-xl flex flex-col gap-3 lg:gap-4 hover:border-zinc-700 transition-colors">
            ...
            <div>
              <h3 className="text-zinc-200 font-medium mb-1 text-sm lg:text-base">{t('workflow.step1.title')}</h3>
              <p className="text-zinc-500 text-xs lg:text-sm">{t('workflow.step1.desc')}</p>
            </div>
          </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 p-4 lg:p-6 rounded-xl flex flex-col gap-3 lg:gap-4 hover:border-emerald-500/30 transition-colors relative overflow-hidden">
            ...
            <div>
              <h3 className="text-white font-medium mb-1 text-sm lg:text-base">{t('workflow.step2.title')}</h3>
              <p className="text-zinc-500 text-xs lg:text-sm">{t('workflow.step2.desc')}</p>
            </div>
          </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 p-4 lg:p-6 rounded-xl flex flex-col gap-3 lg:gap-4 hover:border-zinc-700 transition-colors">
            ...
            <div>
              <h3 className="text-zinc-200 font-medium mb-1 text-sm lg:text-base">{t('workflow.step3.title')}</h3>
              <p className="text-zinc-500 text-xs lg:text-sm">{t('workflow.step3.desc')}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {activeTab === 'projects' ? (
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
          <h2 className="text-zinc-300 font-semibold flex items-center gap-2 text-sm">
            <Clock size={16} /> {t('gallery.title')}
            {filteredProjects.length !== projects.length && (
              <span className="text-zinc-500 font-normal">
                ({filteredProjects.length} {tCommon('of')} {projects.length})
              </span>
            )}
          </h2>
                ...
                >
                  <div className="aspect-square bg-white p-3 lg:p-4 flex items-center justify-center relative">
                    ...
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors z-20 flex flex-col items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                      ...
                      <span className="text-white text-xs font-medium drop-shadow-lg mt-1">{t('gallery.clickToEnlarge')}</span>
                    </div>
                  </div>
              ...
              <div className="flex items-center justify-center gap-4 mt-6">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-zinc-500">
                    {currentPage * PROJECTS_PER_PAGE + 1}-{Math.min((currentPage + 1) * PROJECTS_PER_PAGE, filteredProjects.length)} {tCommon('of')} {filteredProjects.length}
                  </span>
                </div>
            ...
            <div className="flex items-center justify-between p-4 border-b border-zinc-800">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-zinc-500 text-xs mt-0.5">
                  {new Date(selectedProject.created_at).toLocaleDateString(locale)} 
